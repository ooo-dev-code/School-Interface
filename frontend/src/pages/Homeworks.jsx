...
function Homeworks() {
    ...
    const handleSubmit = async (e) => {
        ...
        await axios.post('http://localhost:5000/homework', homeworkData)
            .then((response) => {
                console.log(response.data)
  ...
  return (
    <div>
      <form onSubmit={handleSubmit}>
        <h1 onClick={get}>Add Homeworks</h1>
        <label htmlFor="description">Description:</label>
        <input type="text" id="description" value={description} onChange={(e) => setDescription(e.target.value)} required />

        <label htmlFor="classes">Classes:</label>
        <input type="text" id="classes" value={classes} onChange={(e) => setClasses(e.target.value)} required />
      ...
      {homework.map((hw) => {
        return (
          <table key={hw._id} border="1" style={{ margin: "10px 0", width: "100%" }}>
            <thead>
              <tr>
          <th>Description</th>
          <th>Classes</th>
